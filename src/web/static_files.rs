...
impl StaticFileRegistry {
    pub fn register<P: AsRef<Path>>(&mut self, key: &str, extension: &str, file: P) -> Result<(), std::io::Error> {
        ...
        self.files.insert(to_hash_key(hash) + "." + extension, Bytes::from(buf));
        self.by_key.insert(key.to_owned(), to_hash_key(hash) + "." + extension);

        tracing::info!("Registered '{}' with extension '{}' and hash '{}'", key, extension, to_hash_key(hash));
    ...
    pub fn get_bytes_from_key(&self, key: &str) -> Option<Bytes> {
        self.files.get(key).map(|x| x.clone())
    }
}
